...
member_notifications_router = APIRouter()
...
@member_notifications_router.get("", response_model=ResponseModel)
...
@member_notifications_router.patch("/{notification_id}/read", response_model=ResponseModel)
async def mark_read(notification_id: UUID, request: Request, _=Depends(_require_customer)):
    user_id = request.state.user_payload["sub"]
    nq = NotificationQuery()
...
@member_notifications_router.patch("/read-all", response_model=ResponseModel)
async def mark_all_read(request: Request, _=Depends(_require_customer)):
    user_id = request.state.user_payload["sub"]
    nq = NotificationQuery()
...
@member_notifications_router.delete("/{notification_id}", response_model=ResponseModel)
async def delete_notification(notification_id: UUID, request: Request, _=Depends(_require_customer)):
    user_id = request.state.user_payload["sub"]
    nq = NotificationQuery()
