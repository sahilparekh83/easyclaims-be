...
def _user_to_response(user, user_service: UserService) -> dict:
    ...
    is_superadmin = user_type == "SUPERADMIN" or "SUPERADMIN" in roles
...
async def list_users(request: Request, skip: int = 0, limit: int = 100, _=Depends(require_permission("users", "view"))):
    svc = UserService()
...
async def create_user(body: UserCreate, request: Request, _=Depends(require_permission("users", "add"))):
    svc = UserService()
...
async def get_user(user_id: UUID, request: Request, _=Depends(require_permission("users", "view"))):
    svc = UserService()
...
async def update_user(user_id: UUID, body: UserUpdate, request: Request, _=Depends(require_permission("users", "edit"))):
    svc = UserService()
...
async def delete_user(user_id: UUID, request: Request, _=Depends(require_permission("users", "delete"))):
    svc = UserService()
...
async def assign_role(user_id: UUID, body: AssignRoleRequest, request: Request, _=Depends(require_permission("users", "edit"))):
...
async def remove_role(user_id: UUID, role_id: UUID, request: Request, _=Depends(require_permission("users", "edit"))):
